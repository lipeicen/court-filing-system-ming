...
with sync_playwright() as p:
    ...
    # dump component tree with data keys
    tree = popup.evaluate("""() => {
        function walk(v, depth=0) {
            if (!v) return null;
            const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name);
            const data = v._data ? Object.keys(v._data) : [];
            const children = (v.$children || []).map(walk).filter(Boolean);
            return {tag, data, children};
        }
        return walk(document.querySelector('uni-app').__vue__);
    }""")
    with open(SAVE_DIR / 'wsla_comp_tree.json','w',encoding='utf-8') as f:
