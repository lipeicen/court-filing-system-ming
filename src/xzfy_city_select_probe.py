...
def main():
    ...
    with sync_playwright() as p:
        ...
        city_state = page.evaluate("""() => {
            ...
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue);
            return {value: v.value, cityList: v.cityList.map(c=> ({text: c.citymc, value: c.cityid}))};
        }""")
        print(json.dumps(city_state, ensure_ascii=False, indent=2))
        # select index 1 (maybe 杭州市)
        page.evaluate("""() => {
            const app = document.querySelector('uni-app');
            const vue = app.__vue__;
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            find(vue).onClickItem(1);
        }""")
        wait(5)
        state = page.evaluate("""() => {
            const app = document.querySelector('uni-app');
            ...
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue);
            return {value: v.value, citymc: v.citymc, cityList: v.cityList.map(c=> ({text: c.citymc, value: c.cityid})), fyList: v.fyList.map(f=> ({text: f.text, value: f.value})), ajlxList: v.ajlxList.map(a=> ({text: a.text, value: a.value}))};
        }""")
        print(json.dumps(state, ensure_ascii=False, indent=2))
        with open(SAVE_DIR / 'xzfy_city1_state.json', 'w', encoding='utf-8') as f:
