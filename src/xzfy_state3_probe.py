    ...
    with sync_playwright() as p:
        ...
        state = page.evaluate("""() => {
            ...
            return {
                ...
                value: v.value, citymc: v.citymc, cityListLen: v.cityList ? v.cityList.length : -1,
                fyListLen: v.fyList ? v.fyList.length : -1,
                ajlxListLen: v.ajlxList ? v.ajlxList.length : -1,
                sqrSf: v.sqrSf, sqrSfListLen: v.sqrSfList ? v.sqrSfList.length : -1,
                pcSqrLx: v.pcSqrLx, pcSqrLxListLen: v.pcSqrLxList ? v.pcSqrLxList.length : -1,
                selecedAjlx: v.selecedAjlx, fyId: v.fyId, ajcx: v.ajcx, layyParam: v.layyParam,
                keys: Object.keys(v.$data || v)
            };
        }""")
        print(json.dumps(state, ensure_ascii=False, indent=2)[:4000])
        with open(SAVE_DIR / 'xzfy_state3.json', 'w', encoding='utf-8') as f:
