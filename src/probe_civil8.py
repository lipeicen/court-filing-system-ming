...
def main():
    ...
    with sync_playwright() as p:
        ...
        if item_info and item_info.get('item'):
            ...
            wait(2)
            # Walk Vue tree and collect component names + data keys
            tree = popup.evaluate("""() => {
                function walk(v, depth) {
                    if (!v || depth > 10) return null;
                    const name = v.$options ? (v.$options.name || v.$options._componentTag) : null;
                    const data = v.$data ? Object.keys(v.$data) : null;
                    const props = v.$props ? Object.keys(v.$props) : null;
                    const children = (v.$children || []).map(c => walk(c, depth+1)).filter(Boolean);
                    return {name, data: data ? data.slice(0,30) : null, props: props ? props.slice(0,30) : null, children: children.slice(0,5)};
                }
                const root = document.__vue__ || document.querySelector('uni-app').__vue__;
                return walk(root, 0);
            }""")
            with open(SAVE_DIR / 'vue_tree.json', 'w', encoding='utf-8') as f:
                json.dump(tree, f, ensure_ascii=False, indent=2)
            print('vue_tree saved')
        browser.close()
