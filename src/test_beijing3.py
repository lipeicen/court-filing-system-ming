...
with sync_playwright() as p:
    ...
    # set province to beijing in commonHeader and xzfy
    res = popup.evaluate("""() => {
        ...
        if (!h || !x) return {err: 'missing', hasHeader: !!h, hasXzfy: !!x};
        // 1. set header province
        h.currentAreaIndex = 1;
        // 2. set storage
        uni.setStorageSync('provinceId', '110000');
        // 3. trigger xzfy city reload
    ...
    print('province set result', res)
    wait(5)

    # check cityList and fyList
    res2 = popup.evaluate("""() => {
        function findXzfy(v) {
            if (!v) return null;
            const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name);
        ...
        const x = findXzfy(document.querySelector('uni-app').__vue__);
        return {
            cityList: x.cityList ? x.cityList.slice(0,10) : [],
            fyList: x.fyList ? x.fyList.slice(0,10) : []
        };
    }""")
    print('after set', res2)
    popup.screenshot(path=str(SAVE_DIR / 'court_beijing3.png'), full_page=True)
    with open(SAVE_DIR / 'court_beijing3.html','w',encoding='utf-8') as f: f.write(popup.content())
    print('saved')
    browser.close()
