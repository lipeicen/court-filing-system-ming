    ...
    with sync_playwright() as p:
        ...
        if item_info and item_info.get('item'):
            ...
            popup.evaluate("""() => {
                ...
                const v = walk(document.querySelector('uni-app').__vue__); v.onClickItem(1);
            }""")
            wait(2)
            # Get list of courts
            fyList = popup.evaluate("""() => {
                function walk(v) { if (!v) return null; if (v.$options \u0026\u0026 (v.$options.name || v.$options._componentTag) === 'xzfy') return v; for (const c of v.$children || []) { const r = walk(c); if (r) return r; } return null; }
                const v = walk(document.querySelector('uni-app').__vue__);
                return v.fyList.map(f => ({text: f.text, value: f.value}));
            }""")
            out = []
            for court in fyList:
                popup.evaluate(f"""() => {{
                ...
                out.append({'court': court['text'], 'value': court['value'], 'state': state})
                print(court['text'], state['fyId'], len(state['ajlxList']))
            with open(SAVE_DIR / 'court_ajlx_sequential.json', 'w', encoding='utf-8') as f:
                json.dump(out, f, ensure_ascii=False, indent=2)
            print('saved court_ajlx_sequential')
        browser.close()
