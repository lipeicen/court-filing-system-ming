...
def main():
    ...
    with sync_playwright() as p:
        ...
        wait(4)
        # Get visible text in picker area
        info = page.evaluate("""() => {
            const container = document.querySelector('.uni-picker-container') || document.querySelector('.uni-picker-custom') || document.body;
            const text = Array.from(container.querySelectorAll('*')).map(e => e.textContent.trim()).filter(t => t && t.length < 20);
            return [...new Set(text)].slice(0,100);
        }""")
        with open(SAVE_DIR / 'picker_text.json', 'w', encoding='utf-8') as f:
            json.dump(info, f, ensure_ascii=False, indent=2)
        print('picker texts:', info[:30])
        # Try click 北京市
        if not click_text(page, "北京市", exact=True):
            print('click 北京市 failed')
        wait(3)
        page.screenshot(path=str(SAVE_DIR/'after_beijing.png'))
        # Click 第二级
        info2 = page.evaluate("""() => {
            const container = document.querySelector('.uni-picker-container') || document.querySelector('.uni-picker-custom') || document.body;
            const text = Array.from(container.querySelectorAll('*')).map(e => e.textContent.trim()).filter(t => t && t.length < 20);
        ...
        print('after beijing texts:', info2[:30])
        wait(10)
        browser.close()
