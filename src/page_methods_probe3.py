...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_selector(".fd-children-item:has-text('民事一审')", timeout=15000)
        info = page.evaluate("""() => {
            ...
            if (!v) return {err: 'page not found', tags: []};
            const item0 = v.typeDataList ? v.typeDataList[0] : null;
            return {
                methods: Object.keys(v.$options.methods || {}),
                selected: v.selected, selectedType: v.selectedType, selectedChild: v.selectedChild,
                typeDataListLen: v.typeDataList ? v.typeDataList.length : -1,
                item0Keys: item0 ? Object.keys(item0) : null,
                firstChild: item0 && item0.children ? item0.children[0] : null
            };
        }""")
