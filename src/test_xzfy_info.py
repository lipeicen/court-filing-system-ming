...
with sync_playwright() as p:
    ...
    info = popup.evaluate("""() => {
        ...
        return {
            methods: methods,
            data: v._data ? Object.keys(v._data) : [],
            fyList: v.fyList ? v.fyList.slice(0,5) : [],
            cityList: v.cityList ? v.cityList.slice(0,5) : [],
            currentArea: v.$root.$children[0].$children[0] ? v.$root.$children[0].$children[0].currentAreaIndex : null,
            provinces: v.$root.$children[0].$children[0] ? (v.$root.$children[0].$children[0].provinces || []).slice(0,5) : []
        };
    }""")
    with open(SAVE_DIR / 'xzfy_info.json','w',encoding='utf-8') as f:
        json.dump(info, f, ensure_ascii=False, indent=2)
    print('xzfy info saved', info.keys())
    browser.close()
