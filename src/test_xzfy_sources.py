...
with sync_playwright() as p:
    ...
    info = popup.evaluate("""() => {
        ...
        const methods = x.$options.methods || {};
        const src = {};
        for (const k of ['init','getCityList','getFyList','changeFy','searchFy','nextStep','checkData']) {
            try { src[k] = methods[k].toString(); } catch(e) { src[k] = String(e); }
        }
        return src;
    }""")
    with open(SAVE_DIR / 'xzfy_method_sources.json','w',encoding='utf-8') as f:
