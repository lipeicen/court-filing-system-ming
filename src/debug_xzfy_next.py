...
with sync_playwright() as p:
    ...
    wait(5)
    # list all functions/keys on xzfy
    keys = popup.evaluate("""() => {
        ...
        const all = Object.keys(x).filter(k=>k[0]!=='_' && k!=='$' && k!=='$parent' && k!=='$children' && k!=='$refs' && k!=='$options');
        return all;
    }""")
    print('keys', json.dumps(keys, ensure_ascii=False))
    # read state
    def state():
        return popup.evaluate("""() => {
            function walk(v) { if(!v)return null; const tag=v.$options&&(v.$options.name||v.$options._componentTag||v.$options.__name); if(tag==='xzfy') return v; for(const c of v.$children||[]){ const r=walk(c); if(r)return r;} return null;}
            const x=walk(document.querySelector('uni-app').__vue__);
            return {fyId:x.fyId, fymc:x.fymc, ajlxList:(x.ajlxList||[]).map(a=>({id:a.id,mc:a.mc})), currentIndex:x.currentIndex, value:x.value};
        }""")
    print('state', state())
    # set court directly
    popup.evaluate("""() => {
    ...
    wait(2); print('after set', state())
    # try getAjlxByFyid
    print('call getAjlxByFyid', popup.evaluate("""() => {
        function walk(v) { if(!v)return null; const tag=v.$options&&(v.$options.name||v.$options._componentTag||v.$options.__name); if(tag==='xzfy') return v; for(const c of v.$children||[]){ const r=walk(c); if(r)return r;} return null;}
        const x=walk(document.querySelector('uni-app').__vue__);
        try { x.getAjlxByFyid(); return 'called'; } catch(e) { return 'err: '+e; }
    }"""))
    wait(3); print('after getAjlxByFyid', state())
    # try nextStep
    print('call nextStep', popup.evaluate("""() => {
        function walk(v) { if(!v)return null; const tag=v.$options&&(v.$options.name||v.$options._componentTag||v.$options.__name); if(tag==='xzfy') return v; for(const c of v.$children||[]){ const r=walk(c); if(r)return r;} return null;}
        const x=walk(document.querySelector('uni-app').__vue__);
        try { const r = x.nextStep(); return {ok:true, res:r}; } catch(e) { return {err:String(e)}; }
    }"""))
    wait(3); print('after nextStep', state())
    popup.screenshot(path=str(SAVE_DIR/'xzfy_setcourt.png'))
    browser.close()
