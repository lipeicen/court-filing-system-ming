...
def main():
    ...
    with sync_playwright() as p:
        ...
        if item_info and item_info.get('item'):
            ...
            # find xzfy and get its methods + source of some methods
            xzfy = popup.evaluate("""() => {
                function walk(v) {
                    if (!v) return null;
                ...
                return {
                    methods: methods,
                    source: Object.fromEntries(methods.slice(0,10).map(m => [m, (v.$options.methods[m].toString() || '').slice(0,500)]))
                };
            }""")
            with open(SAVE_DIR / 'xzfy_methods.json', 'w', encoding='utf-8') as f:
                json.dump(xzfy, f, ensure_ascii=False, indent=2)
            print('xzfy_methods saved')
        browser.close()
