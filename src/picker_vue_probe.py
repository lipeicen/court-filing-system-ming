...
def main():
    ...
    with sync_playwright() as p:
        ...
        wait(5)
        # Find the picker element and get its __vue__ component
        picker = page.evaluate("""() => {
            const el = document.querySelector('.uni-picker-container') || document.querySelector('.uni-picker-custom') || document.querySelector('.uni-picker-select');
            if (!el) return {err: 'no picker el'};
            const v = el.__vue__;
            if (!v) return {err: 'no vue'};
            return {tag: v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name), keys: Object.keys(v.$data || v).slice(0,40)};
        }""")
        print(json.dumps(picker, ensure_ascii=False, indent=2))
        # Try to get the page component that has range/data
        all = page.evaluate("""() => {
            ...
            function collect(v) {
                ...
                const keys = Object.keys(v.$data || v);
                const res = [{tag, keys: keys.slice(0,60)}];
                for (const c of v.$children || []) res.push(...collect(c));
                return res;
            }
            return collect(vue);
        }""")
        with open(SAVE_DIR / 'all_after_click.json', 'w', encoding='utf-8') as f:
            json.dump(all, f, ensure_ascii=False, indent=2)
        for c in all:
            if c['tag'] and ('picker' in c['tag'] or 'select' in c['tag'] or 'xzfy' in c['tag']):
                print(c)
        wait(10)
        browser.close()
