...
def main():
    ...
    with sync_playwright() as p:
        ...
        wait(3)
        # Click 民事一审 to open picker
        click_text(page, "民事一审", exact=True)
        wait(4)
        # Inspect parent component in detail
        info = page.evaluate("""() => {
            ...
            const page = findPage(v);
            const picker = document.querySelector('uni-picker').__vue__;
            const parent = picker.$parent;
            return {
                page: page ? {methods: Object.keys(page.$options.methods || {}), data: Object.keys(page.$data), tag: page.$options.name || page.$options._componentTag} : null,
                picker: {methods: Object.keys(picker.$options.methods || {}), data: Object.keys(picker.$data)},
                parent: parent ? {tag: parent.$options.name || parent.$options._componentTag, methods: Object.keys(parent.$options.methods || {}), data: Object.keys(parent.$data)} : null,
                parent_vnode: parent && parent.$vnode ? {tag: parent.$vnode.tag, data_on: parent.$vnode.data && parent.$vnode.data.on ? Object.keys(parent.$vnode.data.on) : null} : null,
                picker_listeners: picker.$listeners ? Object.keys(picker.$listeners) : null
            };
        }""")
        with open(SAVE_DIR/'parent_methods.json','w',encoding='utf-8') as f:
