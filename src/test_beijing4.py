...
with sync_playwright() as p:
    ...
    # change area to Beijing via commonHeader.changeArea(1)
    res = popup.evaluate("""() => {
        ...
        if (typeof h.changeArea === 'function') { h.changeArea(1); return 'changeArea called'; }
        if (h.$options.methods.changeArea) { h.$options.methods.changeArea.call(h, 1); return 'method called'; }
        return 'no changeArea';
    }""")
    print('changeArea', res)
    wait(5)
    ...
    print('after changeArea', res2)
    popup.screenshot(path=str(SAVE_DIR / 'court_beijing4.png'), full_page=True)
    with open(SAVE_DIR / 'court_beijing4.html','w',encoding='utf-8') as f: f.write(popup.content())
    print('saved')
    browser.close()
