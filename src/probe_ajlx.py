...
def main():
    ...
    with sync_playwright() as p:
        ...
        if item_info and item_info.get('item'):
            ...
            # Inspect xzfy data and call methods step by step
            res = popup.evaluate("""() => {
                function walk(v) {
                    if (!v) return null;
                ...
                v.onClickItem(1); // Hangzhou
                return {msg: 'clicked city'};
            }""")
            wait(2)
            res2 = popup.evaluate("""() => {
                function walk(v) {
                ...
                const v = walk(document.querySelector('uni-app').__vue__);
                return {fyList: v.fyList.map(f => ({text: f.text, value: f.value})), selecedAjlx: v.selecedAjlx, ajlxList: v.ajlxList};
            }""")
            with open(SAVE_DIR / 'after_city.json', 'w', encoding='utf-8') as f:
                json.dump(res2, f, ensure_ascii=False, indent=2)
            print('after_city', json.dumps(res2, ensure_ascii=False)[:500])
            # Try selecting first district court and inspect ajlx
            target = popup.evaluate("""() => {
                ...
                const v = walk(document.querySelector('uni-app').__vue__);
                const court = v.fyList.find(f => f.text.includes('上城区'));
                v.fyId = court.value;
                v.changeFy();
                return {court: court.text, value: court.value};
            }""")
            wait(2)
            res3 = popup.evaluate("""() => {
                function walk(v) {
                ...
                const v = walk(document.querySelector('uni-app').__vue__);
                return {fyId: v.fyId, selecedAjlx: v.selecedAjlx, ajlxList: v.ajlxList, msgs: window.__last_toasts || []};
            }""")
            with open(SAVE_DIR / 'after_court.json', 'w', encoding='utf-8') as f:
                json.dump(res3, f, ensure_ascii=False, indent=2)
            print('after_court', json.dumps(res3, ensure_ascii=False)[:500])
        browser.close()
