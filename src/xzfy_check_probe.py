...
def main():
    ...
    with sync_playwright() as p:
        ...
        wait(8)
        # init, select city, select court, set sqrSf, call changeFy
        page.evaluate("""() => {
        ...
        wait(5)
        # Set fyId and sqrSf, call changeFy
        page.evaluate("""() => {
            const app = document.querySelector('uni-app');
            const vue = app.__vue__;
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
        ...
        state = page.evaluate("""() => {
            ...
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue);
            return {fyId: v.fyId, selecedAjlx: v.selecedAjlx, ajlxList: v.ajlxList.map(a=> ({text: a.text, value: a.value})), sqrSf: v.sqrSf, pcSqrLx: v.pcSqrLx, citymc: v.citymc, value: v.value};
        }""")
        print(json.dumps(state, ensure_ascii=False, indent=2))
        with open(SAVE_DIR / 'xzfy_fy_set.json', 'w', encoding='utf-8') as f:
        ...
        check = page.evaluate("""() => {
            ...
            // capture toasts
            const msgs = [];
            const orig = uni.showToast;
            uni.showToast = (opts) => { msgs.push(opts); return orig(opts); };
            v.checkData();
            uni.showToast = orig;
            return {msgs: msgs, afterFyId: v.fyId, afterSelecedAjlx: v.selecedAjlx};
        }""")
        print('checkData', json.dumps(check, ensure_ascii=False, indent=2))
        wait(3)
        # see wsla currentStep
        step = page.evaluate("""() => {
            const app = document.querySelector('uni-app');
            const vue = app.__vue__;
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'pagesWsla-common-wsla-index') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue);
            return v ? v.currentStep : null;
        }""")
