...
def main():
    ...
    with sync_playwright() as p:
        ...
        if item_info and item_info.get('item'):
            ...
            wait(2)
            # Test each court's ajlx
            results = popup.evaluate("""() => {
                function walk(v) {
                    if (!v) return null;
                    if (v.$options && (v.$options.name || v.$options._componentTag) === 'xzfy') return v;
                    for (const c of v.$children || []) { const r = walk(c); if (r) return r; }
                ...
                const v = walk(document.querySelector('uni-app').__vue__);
                const out = [];
                for (const court of v.fyList) {
                    v.fyId = court.value;
                    v.changeFy();
                    // Wait synchronously? changeFy is async but we can check after a tiny delay not possible here; just record fyId
                    out.push({court: court.text, value: court.value});
                }
                return out;
            }""")
            wait(5)  # let last API finish
            final = popup.evaluate("""() => {
                function walk(v) {
                    if (!v) return null;
                    if (v.$options && (v.$options.name || v.$options._componentTag) === 'xzfy') return v;
                    for (const c of v.$children || []) { const r = walk(c); if (r) return r; }
                ...
                const v = walk(document.querySelector('uni-app').__vue__);
                return {fyId: v.fyId, ajlxList: v.ajlxList};
            }""")
            with open(SAVE_DIR / 'court_ajlx_test.json', 'w', encoding='utf-8') as f:
                json.dump({"results": results, "final": final}, f, ensure_ascii=False, indent=2)
            print('court_ajlx_test saved')
        browser.close()
