...
def main():
    ...
    with sync_playwright() as p:
        ...
        wait(5)
        # The picker is on the same page. The component we see is not xzfy? Wait earlier we saw xzfy in direct wsla page. On pick-case-type page, the picker is rendered differently? Let's find all components with data keys range or cityList.
        comps = page.evaluate("""() => {
            ...
            function collect(v) {
                ...
                const res = [{tag, keys: keys.slice(0,40)}];
                for (const c of v.$children || []) res.push(...collect(c));
                return res;
            }
            return collect(vue);
        }""")
        with open(SAVE_DIR / 'picker_components.json', 'w', encoding='utf-8') as f:
            json.dump(comps, f, ensure_ascii=False, indent=2)
        for c in comps:
            if c['tag'] and ('xzfy' in c['tag'] or 'pick' in c['tag'] or 'wsla' in c['tag']):
                print(c)
        wait(10)
        browser.close()
