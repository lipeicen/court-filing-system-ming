...
def main():
    ...
    with sync_playwright() as p:
        ...
        wait(5)
        # The picker is open. Let's inspect the Vue state of picker to get all provinces.
        picker_state = page.evaluate("""() => {
            ...
            return {
                value: v.value, range: v.range.map(r => ({text: r.text, value: r.value, disable: r.disable})),
                cityList: v.cityList.map(c=> ({text: c.citymc, value: c.cityid})),
                fyList: v.fyList.map(f=> ({text: f.text, value: f.value}))
            };
        }""")
        print(json.dumps(picker_state, ensure_ascii=False, indent=2))
        with open(SAVE_DIR / 'picker_state.json', 'w', encoding='utf-8') as f:
            json.dump(picker_state, f, ensure_ascii=False, indent=2)
        # Try to click 广东省 in the picker to see if we can change province
        click_text(page, "广东省")
        wait(1)
        click_text(page, "完成")
        wait(3)
        after_state = page.evaluate("""() => {
            ...
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue);
            return {value: v.value, citymc: v.citymc, cityList: v.cityList.map(c=> ({text: c.citymc, value: c.cityid})), fyList: v.fyList.map(f=> ({text: f.text, value: f.value}))};
        }""")
