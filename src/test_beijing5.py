...
with sync_playwright() as p:
    ...
    # set area directly via commonHeader data and call updateHeaderStorage
    res = popup.evaluate("""() => {
        ...
        const h = findHeader(document.querySelector('uni-app').__vue__);
        if (!h) return 'no header';
        h.currentAreaIndex = 1;
        if (typeof h.updateHeaderStorage === 'function') h.updateHeaderStorage();
        else if (h.$options.methods.updateHeaderStorage) h.$options.methods.updateHeaderStorage.call(h);
        return 'done';
    }""")
    print('storage update', res)
    wait(2)

    # trigger xzfy reload city/fy
    res2 = popup.evaluate("""() => {
        function findXzfy(v) {
            if (!v) return null;
            const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name);
        ...
        if (!x) return 'no xzfy';
        if (typeof x.getCityList === 'function') x.getCityList('110000');
        else if (x.$options.methods.getCityList) x.$options.methods.getCityList.call(x, '110000');
        return 'getCityList called';
    }""")
    print('city reload', res2)
    wait(5)
    ...
    res3 = popup.evaluate("""() => {
        ...
        const x = findXzfy(document.querySelector('uni-app').__vue__);
        return { cityList: x.cityList.slice(0,10), fyList: x.fyList.slice(0,10) };
    }""")
    print('after', res3)
    popup.screenshot(path=str(SAVE_DIR / 'court_beijing5.png'), full_page=True)
    with open(SAVE_DIR / 'court_beijing5.html','w',encoding='utf-8') as f: f.write(popup.content())
    print('saved')
    browser.close()
