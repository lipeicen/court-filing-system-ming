...
def main():
    ...
    with sync_playwright() as p:
        ...
        page = ctx.new_page()
        page.on('response', lambda response: print('RESP', response.status, response.url[:200]) if 'ajlx' in response.url or 'court' in response.url or 'city' in response.url else None)
        ...
        state = page.evaluate("""() => {
            ...
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue);
            return {fyId: v.fyId, selecedAjlx: v.selecedAjlx, ajlxList: v.ajlxList.map(a=> ({text: a.text, value: a.value})), citymc: v.citymc, value: v.value};
        }""")
