...
def main():
    ...
    with sync_playwright() as p:
        ...
        state = page.evaluate("""() => {
            ...
            const wsla = find(vue, 'pagesWsla-common-wsla-index');
            const xzfy = find(vue, 'xzfy');
            return {
                wsla_methods: wsla ? Object.keys(wsla.$options.methods || {}) : null,
                wsla_currentStep: wsla ? wsla.currentStep : null,
                wsla_ajlx: wsla ? wsla.ajlx : null,
                wsla_layyId: wsla ? wsla.layyId : null,
                xzfy_methods: xzfy ? Object.keys(xzfy.$options.methods || {}) : null,
                xzfy_cityListLen: xzfy && xzfy.cityList ? xzfy.cityList.length : null,
                xzfy_fyListLen: xzfy && xzfy.fyList ? xzfy.fyList.length : null,
                xzfy_value: xzfy ? xzfy.value : null,
                xzfy_sqrSfList: xzfy && xzfy.sqrSfList ? xzfy.sqrSfList.length : null
            };
        }""")
        print(json.dumps(state, ensure_ascii=False, indent=2))
        with open(SAVE_DIR / 'wsla_xzfy_state.json', 'w', encoding='utf-8') as f:
