...
def main():
    ...
    with sync_playwright() as p:
        ...
        wait(3)
        save_state(page, "pick_case_type2")
        ...
        print('item_info', json.dumps(item_info, ensure_ascii=False))
        if item_info and item_info.get('item'):
            ...
            for _ in range(30):
                wait(1)
                state = popup.evaluate("() => document.readyState")
                print('readyState', state, 'url', popup.url)
                if state == 'complete':
                    break
            save_state(popup, "civil_first_loaded")
            # Also try taking non-full-page screenshot in case full_page blank
            popup.screenshot(path=str(SAVE_DIR / "civil_first_viewport.png"))
            print('saved viewport')
        browser.close()
