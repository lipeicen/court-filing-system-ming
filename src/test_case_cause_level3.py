...
with sync_playwright() as p:
    ...
    time.sleep(3)
    popup.locator("text=合同、准合同纠纷").first.click(timeout=3000)
    time.sleep(2)
    popup.locator("text=合同纠纷").first.click(timeout=3000)
    time.sleep(2)
    popup.screenshot(path='screenshots/probe/case_cause_level3.png')
    texts = popup.evaluate("""() => {
        const out = [];
        for (const el of document.querySelectorAll('.fd-item, .item-text, .fd-tree-node, .uni-list-item, .leaf-node, .is-leaf, .selected-item')) {
