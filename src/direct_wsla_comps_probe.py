    ...
    with sync_playwright() as p:
        ...
        comps = page.evaluate("""() => {
            ...
            function collect(v) {
                ...
                const res = [{tag, keys: Object.keys(v.$data || v).slice(0,30)}];
                for (const c of v.$children || []) res.push(...collect(c));
                return res;
            }
            return collect(vue);
        }""")
        with open(SAVE_DIR / 'direct_wsla_comps.json', 'w', encoding='utf-8') as f:
            json.dump(comps, f, ensure_ascii=False, indent=2)
        for c in comps:
            if c['tag'] and ('xzfy' in c['tag'] or 'wsla' in c['tag'] or 'common' in c['tag']):
                print(c)
        wait(10)
        browser.close()
