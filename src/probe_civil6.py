...
def main():
    ...
    with sync_playwright() as p:
        ...
        if item_info and item_info.get('item'):
            ...
            save_state(popup, "p6_step1_initial")
            # click city 杭州市
            click_text(popup, "杭州市")
            wait(2)
            save_state(popup, "p6_step1_hangzhou")
            # Use JS to click the label containing 杭州市上城区人民法院 (a leaf court)
            popup.evaluate("""() => {
                const all = document.querySelectorAll('uni-label');
                for (const el of all) {
                    if (el.textContent.trim() === '杭州市上城区人民法院') {
                        el.click();
            ...
            wait(1)
            save_state(popup, "p6_step1_court_selected")
            # click applicant
            click_text(popup, "本人申请")
            wait(1)
            save_state(popup, "p6_step1_applicant")
            # click next button
            popup.evaluate("""() => {
                const all = document.querySelectorAll('uni-view');
                for (const el of all) if (el.textContent.trim() === '下一步' && el.className.includes('fd-com-btn-container')) { el.click(); return 'next clicked'; }
                // fallback click any element containing 下一步
                for (const el of document.querySelectorAll('*')) if (el.textContent.trim() === '下一步') { el.click(); return 'next fallback'; }
                return 'next not found';
            }""")
            wait(3)
            save_state(popup, "p6_step2_after_next")
