...
with sync_playwright() as p:
    ...
    try:
        popup.evaluate("""() => {
            ...
            for (const el of all) if (el.textContent && el.textContent.includes('综治中心')) {
                let p = el; while (p && p !== document.body) { if (p.classList && p.classList.contains('uni-popup')) { const close = p.querySelector('.uni-popup__close'); if (close) close.click(); else p.style.display='none'; break; } p = p.parentElement; }
            }
        }""")
        wait(1)
    except Exception:
        pass
    popup.screenshot(path=str(SAVE_DIR / 'court_select.png'))
    ...
    wait(5)
    print('popup ready, visible browser')
