...
def main():
    ...
    with sync_playwright() as p:
        ...
        if item_info and item_info.get('item'):
            ...
            # dump page structure via JS
            info = popup.evaluate("""() => {
                const all = document.querySelectorAll('*');
                let textNodes = [];
                for (const el of all) {
                    if (el.children.length === 0 && el.textContent && el.textContent.trim().length > 0) {
                        textNodes.push({tag: el.tagName.toLowerCase(), text: el.textContent.trim().slice(0,50), cls: el.className ? el.className.slice(0,80) : ''});
                    }
                }
                return textNodes.slice(0,200);
            }""")
            with open(SAVE_DIR / 'civil_first_textnodes.json', 'w', encoding='utf-8') as f:
                json.dump(info, f, ensure_ascii=False, indent=2)
            print('saved textnodes')
        browser.close()
