...
def main():
    ...
    with sync_playwright() as p:
        ...
        def on_response(resp):
            if 'ajlx' in resp.url or 'pz/fy' in resp.url:
                try:
                    body = resp.text()
                    ajlx_bodies[resp.url] = body[:2000]
                except Exception as e:
                    ajlx_bodies[resp.url] = 'error: ' + str(e)
        ...
        page.evaluate("""() => {
            const app = document.querySelector('uni-app'); const vue = app.__vue__;
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue); v.onClickItem(1);
        }""")
        wait(5)
        page.evaluate("""() => {
            const app = document.querySelector('uni-app'); const vue = app.__vue__;
            function find(v) { if (!v) return null; const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name); if (tag === 'xzfy') return v; for (const c of v.$children || []) { const r = find(c); if (r) return r; } return null; }
            const v = find(vue); v.fyId = '1302'; v.changeFy();
        }""")
        wait(8)
        with open(SAVE_DIR / 'ajlx_bodies.json', 'w', encoding='utf-8') as f:
            json.dump(ajlx_bodies, f, ensure_ascii=False, indent=2)
        for u, b in ajlx_bodies.items():
            print('URL', u)
            print('BODY', b[:500])
            print('---')
        browser.close()
