...
with sync_playwright() as p:
    ...
    wait(5)
    # full method source and component tree
    info = popup.evaluate("""() => {
        function walk(v) {
            if (!v) return null;
        ...
        const fns = ['change','changeFy','checkboxChange','bindChange','onClickItem','getFyList','nextStep','checkData'];
        const out = {};
        for (const n of fns) {
            try {
                const f = x[n];
                out[n] = f ? f.toString() : 'missing';
            } catch (e) { out[n] = 'err: '+e; }
        }
        out.fyList_first5 = (x.fyList || []).slice(0,5);
        out.ajlxList_len = (x.ajlxList || []).length;
        out.fyId = x.fyId;
        out.fymc = x.fymc;
        return out;
    }""")
    ...
    for k,v in info.items():
        if k not in ('fyList_first5','ajlxList_len','fyId','fymc'):
            print(f'\n### {k} ###')
            print(v)
    print('--- STATE ---')
    print('fyList_first5', info.get('fyList_first5'))
    print('ajlxList_len', info.get('ajlxList_len'))
    print('fyId', info.get('fyId'), 'fymc', info.get('fymc'))
    browser.close()
