...
with sync_playwright() as p:
    ...
    time.sleep(2)
    popup.screenshot(path='screenshots/probe/case_cause_level2.png')
    texts = popup.evaluate("""() => {
        const out = [];
        for (const el of document.querySelectorAll('.fd-item, .item-text, .fd-tree-node, .uni-list-item, .leaf-node, .is-leaf')) {
