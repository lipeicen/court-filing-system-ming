...
with sync_playwright() as p:
    ...
    res = popup.evaluate("""() => {
        ...
        return {keys: keys, hasProvince: 'provinceId' in comp};
    }""")
    print('comp info', res)
    ...
    popup.evaluate("""() => {
        ...
        const comp = find(document.querySelector('uni-app').__vue__);
        comp.provinceId = '110000';
        uni.setStorageSync('provinceId', '110000');
        if (typeof comp.getListData === 'function') comp.getListData();
        else if (comp.$options.methods && comp.$options.methods.getListData) comp.$options.methods.getListData.call(comp);
        return 'done';
    }""")
    wait(5)
    popup.screenshot(path=str(SAVE_DIR / 'court_beijing2.png'), full_page=True)
    with open(SAVE_DIR / 'court_beijing2.html','w',encoding='utf-8') as f: f.write(popup.content())
    print('saved')
    browser.close()
