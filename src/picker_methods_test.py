        ...
        def call(method):
            return page.evaluate(f"""() => {{
                ...
                if (typeof v.{method} === 'function') {{
                    try {{ v.{method}(); return {{ok: true}}; }} catch(e) {{ return {{err: e.message}}; }}
                }} else return {{err: 'no method {method}'}};
            }}""")
        for m in ['_select','_change','_close','_show','_cancel']:
            print('call', m, call(m))
            wait(1)
        # Get state
        state = page.evaluate("""() => {
            ...
            const comp = find(v);
            if (!comp) return null;
            return {keys: Object.keys(comp.$data).slice(0,60), province: comp.$data.province || comp.$data.selected || comp.$data.selectedProvince || comp.$data.value || comp.$data.index};
        }""")
        with open(SAVE_DIR/'after_method_state.json','w',encoding='utf-8') as f:
            json.dump(state, f, ensure_ascii=False, indent=2)
        print('state', json.dumps(state, ensure_ascii=False, indent=2)[:2000])
        # Also check if picker still visible
        visible = page.evaluate("""() => {
            const el = document.querySelector('.uni-picker-container');
            return el ? window.getComputedStyle(el).display : 'none';
        }""")
        print('picker display', visible)
        wait(10)
        browser.close()
