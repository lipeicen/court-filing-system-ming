    ...
    with sync_playwright() as p:
        ...
        el_info = page.evaluate("""() => {
            for (const el of document.querySelectorAll('*')) if (el.textContent.trim()==='北京市') {
                return {tag: el.tagName, cls: el.className, outer: el.outerHTML.slice(0,500)};
            }
            return null;
        }""")
        print('el_info', json.dumps(el_info, ensure_ascii=False, indent=2))
        # click the element via Playwright selector based on tag and class, find index among similar
        if el_info and el_info.get('cls'):
            selector = f"{el_info['tag'].lower()}.{'.'.join(el_info['cls'].split())}"
            print('selector', selector)
            try:
                page.locator(selector).filter(has_text='北京市').click(timeout=5000)
