...
def main():
    ...
    with sync_playwright() as p:
        ...
        # get new page
        pages = ctx.pages
        new_page = pages[-1] if len(pages) > 1 else page
        # inspect wsla page components
        comp_info = new_page.evaluate("""() => {
            const app = document.querySelector('uni-app').__vue__;
            function collect(v) {
                if (!v) return [];
                const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name);
                const res = [{tag, data: Object.keys(v.$data || {}).slice(0,40)}];
                for (const c of v.$children || []) res.push(...collect(c));
                return res;
            }
            return collect(app);
        }""")
        with open(SAVE_DIR/'wsla_components.json','w',encoding='utf-8') as f:
            json.dump(comp_info, f, ensure_ascii=False, indent=2)
        for c in comp_info:
            if c['tag'] and ('xzfy' in c['tag'] or 'wsla' in c['tag'] or 'court' in c['tag']):
                print(c)
        # find xzfy component and inspect methods/data
        xzfy = new_page.evaluate("""() => {
            function walk(v) {
                if (!v) return null;
                const tag = v.$options && (v.$options.name || v.$options._componentTag || v.$options.__name);
            ...
            return {methods: Object.keys(v.$options.methods || {}), data: Object.keys(v.$data || {}), fyList: v.fyList ? v.fyList.map(f => ({text: f.text, value: f.value}))[:20] : null};
        }""")
        with open(SAVE_DIR/'xzfy_info.json','w',encoding='utf-8') as f:
            json.dump(xzfy, f, ensure_ascii=False, indent=2)
        print(json.dumps(xzfy, ensure_ascii=False, indent=2)[:2000])
        wait(10)
        browser.close()
