...
with sync_playwright() as p:
    ...
    # click Haidian court card by DOM
    popup.evaluate("""() => {
        const all = document.querySelectorAll('*');
        for (const el of all) {
            if (el.textContent && el.textContent.trim() === '北京市海淀区人民法院') {
                el.click();
    ...
    popup.screenshot(path=str(SAVE_DIR / 'court_click_haidian.png'), full_page=True)
    with open(SAVE_DIR / 'court_click_haidian.html','w',encoding='utf-8') as f: f.write(popup.content())
    print('saved')
    browser.close()
