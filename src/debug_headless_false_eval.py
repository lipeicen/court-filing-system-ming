...
with sync_playwright() as p:
    browser = p.chromium.launch(headless=False, slow_mo=300, args=["--disable-blink-features=AutomationControlled"])
    ...
    wait(3)
    # set province only via storage, no header update
    page.evaluate("""() => {
    ...
    for i in range(10):
        try:
            r = popup.evaluate("""() => {
                ...
                const x = walk(app.__vue__);
                return x ? {fyId: x.fyId, fyListLen: x.fyList && x.fyList.length} : {err: 'no xzfy'};
            }""")
            print('eval ok', i, r)
            break
        except Exception as e:
            print('eval error', i, e)
        wait(1)
    print('events:')
    for e in events[-10:]:
        print(e)
